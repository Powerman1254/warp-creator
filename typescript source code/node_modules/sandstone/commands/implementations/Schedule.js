"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Schedule = void 0;
const Command_1 = require("../Command");
const decorators_1 = require("../decorators");
const Tag_1 = require("../../resources/Tag");
function isMCFunctionInstance(callback) {
    return Object.prototype.hasOwnProperty.call(callback, 'datapack');
}
class Schedule extends Command_1.Command {
    constructor() {
        super(...arguments);
        this.clear = (func) => {
            if (typeof func === 'string' || func instanceof Tag_1.TagInstance) {
                this.commandsRoot.addAndRegister('schedule', 'clear', func);
            }
            else {
                func.schedule.clear();
            }
        };
        this.function = (...args) => {
            const datapack = this.commandsRoot.Datapack;
            const scheduleCallback = (name, callback, delay, type, asChild) => {
                datapack.createCallbackMCFunction(name, callback, asChild).schedule(delay, type);
            };
            // If a callback with a name has been given, create a root function & schedule it
            if (typeof args[0] === 'string' && typeof args[1] === 'function') {
                const [callbackName, callback, delay, type] = args;
                scheduleCallback(callbackName, callback, delay, type, false);
                return;
            }
            const [func, delay, type] = args;
            if (typeof func === 'string' || func instanceof Tag_1.TagInstance || isMCFunctionInstance(func)) {
                // If a string/tag has been given simply schedule it
                this.commandsRoot.arguments.push('schedule', 'function', func, delay, type);
                this.commandsRoot.register();
            }
            else {
                // If a nameless callback has been given, create a child function & schedule it
                scheduleCallback('_schedule', func, delay, type, true);
            }
        };
    }
}
__decorate([
    decorators_1.command(['schedule', 'clear'], {
        isRoot: true, executable: true, registerArguments: false, hasSubcommands: true,
    }),
    __metadata("design:type", Function)
], Schedule.prototype, "clear", void 0);
__decorate([
    decorators_1.command(['schedule', 'function'], {
        isRoot: true, executable: true, registerArguments: false, hasSubcommands: true,
    }),
    __metadata("design:type", Object)
], Schedule.prototype, "function", void 0);
exports.Schedule = Schedule;
