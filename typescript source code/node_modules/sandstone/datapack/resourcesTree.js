...
class ResourcesTree {
     ...
     */
    addResource(resourceType, resource, conflictStrategy) {
        const parentPath = resource.path.slice(0, -1);
        const handleConflict = () => {
            const previousResource = this.getResourceOrFolder(resource.path, resourceType);
            const toTitleCase = (str) => str.charAt(0).toUpperCase() + str.slice(1);
            const niceResourceType = toTitleCase(resourceType.slice(0, -1));
            const niceName = minecraft_1.toMCFunctionName(resource.path);
            if (!previousResource || conflictStrategy === 'replace' || conflictStrategy === 'warn' || !previousResource.isResource) {
                if (conflictStrategy === 'warn' && previousResource && previousResource.isResource) {
                    console.warn(chalk_1.default.keyword('orange')('Warning:', `Tried to create a ${niceResourceType} named "${minecraft_1.toMCFunctionName(resource.path)}", but found an already existing one.`, "The new one has replaced the old one. To remove this warning, please change the options of the resource to { onConflict: '/* other option */' }."));
                }
            ...
            if (conflictStrategy === 'ignore' || !resource.isResource) {
                ...
                if (conflictStrategy !== 'ignore') {
                    [...resource.children].forEach(([key, child]) => {
                        previousResource.children.set(key, child);
                    });
                }
                return previousResource;
            }
            if (conflictStrategy === 'throw') {
                throw new Error(`Tried to create a ${niceResourceType} named "${minecraft_1.toMCFunctionName(resource.path)}", but found an already existing one.`);
            }
            return conflictStrategy(previousResource, resource);
        };
        ...
        if (parentPath.length >= 2) {
            ...
            parent.children.set(newResource.path[resource.path.length - 1], newResource);
        }
        else {
            ...
            namespaceResource[resourceType].set(newResource.path[1], newResource);
        }
        return resource;
    }
    /**
