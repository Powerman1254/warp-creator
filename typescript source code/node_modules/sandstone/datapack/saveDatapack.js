...
const writeFile = (saveObject) => writeFileAsync(path.join(saveObject.rootPath, saveObject.relativePath), saveObject.content);
    ...
    if (resource.isResource) {
        ...
        if (!options.dryRun) {
            if (!options.customFileHandler && rootPath) {
                filesystem_1.createDirectory(path.join(rootPath, resourceFolder));
            }
...
function getDestinationPath(name, options) {
    if (hasWorld(options) && options.world !== undefined) {
        return path.join(filesystem_1.getWorldPath(options === null || options === void 0 ? void 0 : options.world, options === null || options === void 0 ? void 0 : options.minecraftPath), 'datapacks', name);
    }
    if (hasRoot(options) && options.asRootDatapack !== undefined) {
        return path.join(filesystem_1.getMinecraftPath(), 'datapacks', name);
    }
    ...
    try {
        ...
        for (const n of resources.namespaces.values()) {
            ...
            for (const r of n.item_modifiers.values()) {
                promises.push(...saveResource(rootPath, 'item_modifiers', r, options, (resource) => JSON.stringify(resource.modifier, null, indentation), (namespace, folders, fileName) => `Item modifier ${namespace}:${[...folders, fileName].join('/')}`));
            }
        }
