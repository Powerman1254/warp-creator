...
MCFunction(
	...
	() => {
		...
		execute
			.as(Selector("@e", { type: "minecraft:armor_stand", tag: "Area" }))
			.store.result.score(Selector("@s"), UUID[0].name)
			.run.data.get.entity(Selector("@s"), "UUID[0]");
		execute
			.as(Selector("@e", { type: "minecraft:armor_stand", tag: "Area" }))
			.store.result.score(Selector("@s"), UUID[1].name)
			.run.data.get.entity(Selector("@s"), "UUID[1]");
		execute
			.as(Selector("@e", { type: "minecraft:armor_stand", tag: "Area" }))
			.store.result.score(Selector("@s"), UUID[2].name)
			.run.data.get.entity(Selector("@s"), "UUID[2]");
		execute
			.as(Selector("@e", { type: "minecraft:armor_stand", tag: "Area" }))
			.store.result.score(Selector("@s"), UUID[3].name)
			.run.data.get.entity(Selector("@s"), "UUID[3]");
...
const warp_test = MCFunction("warp_manager:warp_test", () => {
	execute
		.as(Selector("@e", { type: "minecraft:armor_stand", tag: "Warp" }))
		.at(Selector("@a"))
		.run(() => {
			_.if(_.and(_.and(x(Selector("@p")).greaterOrEqualThan(min_x(Selector("@s"))), _.and(x(Selector("@p")).lessOrEqualThan(max_x(Selector("@s"))), _.and(z(Selector("@p")).lessOrEqualThan(max_z(Selector("@s"))), z(Selector("@p")).greaterOrEqualThan(min_z(Selector("@s")))))), _.and(y(Selector("@p")).greaterOrEqualThan(min_y(Selector("@s"))), y(Selector("@p")).lessOrEqualThan(max_y(Selector("@s"))))), () => {
				...
				execute
					.as(Selector("@p"))
					.at(Selector("@e", { type: "minecraft:marker", limit: 1, tag: "warp" }))
					.facing(loc(0, 0, 1))
					.run.tp(Selector("@s"), Selector("@e", { type: "minecraft:marker", limit: 1, tag: "warp" }));
